...
function parseListIndex(key) {
  ...
  if (typeof key !== 'number') {
    throw new TypeError(
      'A list index must be a number, but you passed ' + JSON.stringify(key),
    );
  }
  if (key < 0 || isNaN(key) || key === Infinity || key === -Infinity) {
    throw new RangeError(
      'A list index must be positive, but you passed ' + key,
    );
  }
  return key;
}
...
function listMethods(context, listId) {
  const methods = {
    ...
    fill(value, start, end) {
      ...
      for (
        let index = parseListIndex(start || 0);
        index < parseListIndex(end || list.length);
        index++
      ) {
...
const MapHandler = {
  ...
  has(target, key) {
    const { context, objectId } = target;
    return (
      [OBJECT_ID, CHANGE].includes(key) || key in context.getObject(objectId)
    );
  },
 ...
 */
function instantiateProxy(objectId, context) {
  const object = context.getObject(objectId);
  if (Array.isArray(object) || object instanceof Text) {
    return listProxy(context, objectId);
  } else if (object instanceof Table) {
    return object.getWriteable(context);
  } else {
    return mapProxy(context, objectId);
  }
}

function rootObjectProxy(context) {
  context.instantiateObject = (objectId) => instantiateProxy(objectId, context);
