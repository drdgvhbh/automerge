import { isObject, parseElemId, ROOT_ID } from '../src/common';
import { CONFLICTS, ELEM_IDS, MAX_ELEM, OBJECT_ID } from './constants';
...
function updateInbound(objectId, refsBefore, refsAfter, inbound) {
  for (const ref of Object.keys(refsBefore)) {
    if (!refsAfter[ref]) {
      delete inbound[ref];
    }
  }
  for (const ref of Object.keys(refsAfter)) {
...
function cloneMapObject(originalObject, objectId) {
  if (originalObject && originalObject[OBJECT_ID] !== objectId) {
    throw new RangeError(
      `cloneMapObject ID mismatch: ${
        originalObject[OBJECT_ID]
      } !== ${objectId}`,
    );
  }
  ...
  const conflicts = Object.assign(
    {},
    originalObject ? originalObject[CONFLICTS] : undefined,
  );
  ...
  if (diff.action === 'create') {
    ...
    if (diff.conflicts) {
      ...
      for (const conflict of diff.conflicts) {
        conflicts[diff.key][conflict.actor] = getValue(
          conflict,
          cache,
          updated,
        );
...
function parentMapObject(objectId, cache, updated) {
  ...
  for (const key of Object.keys(object)) {
    ...
    let conflicts = object[CONFLICTS][key] || {},
      conflictsUpdate: any = null;
    for (const actorId of Object.keys(conflicts)) {
...
function updateTableObject(diff, cache, updated, inbound) {
  if (!updated[diff.obj]) {
    updated[diff.obj] = cache[diff.obj]
      ? cache[diff.obj]._clone()
      : instantiateTable(diff.obj);
  }
  const object = updated[diff.obj];
  const refsBefore = {},
    refsAfter = {};
...
function parentTableObject(objectId, cache, updated) {
  ...
  const table = updated[objectId];

  for (const key of Object.keys(table.entries)) {
    const value = table.byId(key);
    if (isObject(value) && updated[value[OBJECT_ID]]) {
...
function cloneListObject(originalList, objectId) {
  if (originalList && originalList[OBJECT_ID] !== objectId) {
    throw new RangeError(
      `cloneListObject ID mismatch: ${originalList[OBJECT_ID]} !== ${objectId}`,
    );
  }
  const list = originalList ? originalList.slice() : []; // slice() makes a shallow clone
  const conflicts =
    originalList && originalList[CONFLICTS]
      ? originalList[CONFLICTS].slice()
      : [];
  const elemIds =
    originalList && originalList[ELEM_IDS]
      ? originalList[ELEM_IDS].slice()
      : [];
  const maxElem =
    originalList && originalList[MAX_ELEM] ? originalList[MAX_ELEM] : 0;
...
function parentListObject(objectId, cache, updated) {
  ...
  for (let index = 0; index < list.length; index++) {
    ...
    let conflicts = list[CONFLICTS][index] || {},
      conflictsUpdate: any = null;
    for (const actorId of Object.keys(conflicts)) {
      value = conflicts[actorId];
      if (isObject(value) && updated[value[OBJECT_ID]]) {
...
function updateTextObject(diffs, startIndex, endIndex, cache, updated) {
  ...
  while (startIndex <= endIndex) {
    ...
    if (diff.action === 'create') {
      ...
      if (
        startIndex === endIndex ||
        diffs[startIndex + 1].action !== 'insert' ||
...
function updateParentObjects(cache, updated, inbound) {
  ...
  while (Object.keys(affected).length > 0) {
    const parents = {};
    for (const childId of Object.keys(affected)) {
    ...
    for (const objectId of Object.keys(parents)) {
...
function applyDiffs(diffs, cache, updated, inbound) {
  ...
  for (let endIndex = 0; endIndex < diffs.length; endIndex++) {
    ...
    if (diff.type === 'map') {
      ...
      if (
        endIndex === diffs.length - 1 ||
        diffs[endIndex + 1].obj !== diff.obj
      ) {
...
export { applyDiffs, updateParentObjects, cloneRootObject };
