...
import { CACHE, CONFLICTS, INBOUND, MAX_ELEM, OBJECT_ID } from './constants';
import { Counter, getWriteableCounter } from './counter';
import { Table } from './table';
import { getElemId, Text } from './text';
...
class Context {
  ...
  createNestedObjects(value) {
    ...
    if (value instanceof Text) {
      ...
      if (value.count > 0) {
        throw new RangeError(
          'Assigning a non-empty Table object is not supported',
        );
      }
  ...
  setMapKey(objectId, type, key, value) {
    if (typeof key !== 'string') {
      throw new RangeError(
        `The key of a map entry must be a string, not ${typeof key}`,
      );
    }
    if (key === '') {
      throw new RangeError(
        'The key of a map entry must not be an empty string',
      );
    }
    ...
    if (
      object[key] !== value ||
      object[CONFLICTS][key] ||
      value === undefined
    ) {
      const valueObj = this.setValue(objectId, key, value);
      this.apply(
        Object.assign({ action: 'set', type, obj: objectId, key }, valueObj),
      );
    }
  }
  ...
  insertListItem(objectId, index, value) {
    ...
    if (index < 0 || index > list.length) {
      throw new RangeError(
        `List index ${index} is out of bounds for list of length ${
          list.length
        }`,
      );
    }
    ...
    this.apply(
      Object.assign(
        { action: 'insert', type, obj: objectId, index, elemId },
        valueObj,
      ),
    );
  ...
  setListIndex(objectId, index, value) {
    ...
    if (
      ...
      const valueObj = this.setValue(objectId, elemId, value);
      this.apply(
        Object.assign({ action: 'set', type, obj: objectId, index }, valueObj),
      );
    }
  }
  ...
  splice(objectId, start, deletions, insertions) {
    ...
    if (deletions > 0) {
      if (start < 0 || start > list.length - deletions) {
        throw new RangeError(
          `${deletions} deletions starting at index ${start} are out of bounds for list of length ${
            list.length
          }`,
        );
      }
